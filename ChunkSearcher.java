...
public final class ChunkSearcher
{
	...
	public void startSearching2(ExecutorService pool)
	{
		if(status != Status.IDLE)
			throw new IllegalStateException();
		
		status = Status.SEARCHING;
		future = pool.submit(this::searchNow2);
	}
		...
		for(int x = minX; x <= maxX; x++)
			for(int y = minY; y <= maxY; y++)
				for(int z = minZ; z <= maxZ; z++)
				{
					...
					block2 = BlockUtils.getBlock(pos2);
					if(block2.equals(Blocks.AIR))exposed = true;
					pos2 = new BlockPos(x-1, y, z);
					block2 = BlockUtils.getBlock(pos2);
					if(block2.equals(Blocks.AIR))exposed = true;
