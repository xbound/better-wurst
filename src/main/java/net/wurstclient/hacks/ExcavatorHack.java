	...
	implements UpdateListener, RenderListener, GUIRenderListener
{
	...
	private final SliderSetting range =
		new SliderSetting("Range", "Ignored in Fill mode." ,5, 2, 6, 0.05, ValueDisplay.DECIMAL);
	...
	public void onRender(MatrixStack matrixStack, float partialTicks)
	{
		...
		// area preview
		if(showarea.isChecked() && area == null && step == Step.END_POS && step.pos != null)
		{
	...
	private void fillall(int x1,int y1,int z1,int x2,int y2,int z2){
		...
		if(step==0){
			...
			//}else {
				...
				if(step>0)
				MC.getNetworkHandler().sendChatCommand("fill " + x1 + " " +  y1 + " " + start + " " + x2 + " " + y2 + " " + Math.min(start+step-1,end) + " " + btf);
				else{
				ChatUtils.warning("Recursive filling may take a lot of time.");
				int xmax=Math.max(x1,x2);
				int xmin=Math.min(x1,x2);
				for(;xmin<xmax;++xmin)
					fillall(xmin,y1,z1,xmin,y2,z2);
				break;
				}
				}
			//}
	...
	private void scanArea()
	{
		int offx,offz;
		// initialize area
		if(mode.getSelected() == Mode.FILL){
		fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		setEnabled(false);
		area=null;
		return;
		}else if(mode.getSelected() == Mode.FILLW || mode.getSelected() == Mode.FILLS){
		if(Math.abs(Step.END_POS.pos.getX()-Step.START_POS.pos.getX())>1 && Math.abs(Step.END_POS.pos.getY()-Step.START_POS.pos.getY())>1 && Math.abs(Step.END_POS.pos.getZ()-Step.START_POS.pos.getZ())>1){
			if(Step.END_POS.pos.getX()>Step.START_POS.pos.getX())offx=1;
			else if(Step.END_POS.pos.getX()<Step.START_POS.pos.getX())offx=-1;
			else offx=0;
			if(Step.END_POS.pos.getZ()>Step.START_POS.pos.getZ())offz=1;
			else if(Step.END_POS.pos.getZ()<Step.START_POS.pos.getZ())offz=-1;
			else offz=0;
			if(mode.getSelected() == Mode.FILLS){
		fillall(Step.START_POS.pos.getX()+offx,Step.START_POS.pos.getY(),Step.START_POS.pos.getZ()+offz,
			Step.END_POS.pos.getX()-offx,Step.START_POS.pos.getY(),Step.END_POS.pos.getZ()-offz);
		fillall(Step.START_POS.pos.getX()+offx,Step.END_POS.pos.getY(),Step.START_POS.pos.getZ()+offz,
			Step.END_POS.pos.getX()-offx,Step.END_POS.pos.getY(),Step.END_POS.pos.getZ()-offz);
			}
		fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.START_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.END_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.END_POS.pos.getY(),Step.START_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.START_POS.pos.getY(),Step.END_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		}else fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		setEnabled(false);
		area=null;
		return;
		}else if(mode.getSelected() == Mode.FILLF){
		if(Math.abs(Step.END_POS.pos.getX()-Step.START_POS.pos.getX())>1 && Math.abs(Step.END_POS.pos.getY()-Step.START_POS.pos.getY())>1 && Math.abs(Step.END_POS.pos.getZ()-Step.START_POS.pos.getZ())>1){
			if(Step.END_POS.pos.getX()>Step.START_POS.pos.getX())offx=1;
			else if(Step.END_POS.pos.getX()<Step.START_POS.pos.getX())offx=-1;
			else offx=0;
			if(Step.END_POS.pos.getY()>Step.START_POS.pos.getY())offz=1;
			else if(Step.END_POS.pos.getY()<Step.START_POS.pos.getY())offz=-1;
			else offz=0;//reuse offz as offy
		fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.END_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.START_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.START_POS.pos.getY(),Step.START_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.START_POS.pos.getY(),Step.END_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.START_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX(),Step.END_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.START_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.END_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.END_POS.pos.getY(),Step.START_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX()+offx,Step.END_POS.pos.getY(),Step.END_POS.pos.getZ(),
			Step.END_POS.pos.getX()-offx,Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY()+offz,Step.END_POS.pos.getZ(),
			Step.START_POS.pos.getX(),Step.END_POS.pos.getY()-offz,Step.END_POS.pos.getZ());
		fillall(Step.END_POS.pos.getX(),Step.START_POS.pos.getY()+offz,Step.END_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY()-offz,Step.END_POS.pos.getZ());
		fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY()+offz,Step.START_POS.pos.getZ(),
			Step.START_POS.pos.getX(),Step.END_POS.pos.getY()-offz,Step.START_POS.pos.getZ());
		fillall(Step.END_POS.pos.getX(),Step.START_POS.pos.getY()+offz,Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY()-offz,Step.START_POS.pos.getZ());
		}else fillall(Step.START_POS.pos.getX(),Step.START_POS.pos.getY(),Step.START_POS.pos.getZ(),
			Step.END_POS.pos.getX(),Step.END_POS.pos.getY(),Step.END_POS.pos.getZ());
		setEnabled(false);
		area=null;
